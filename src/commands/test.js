...
export default function (program) {
  program
    ...
    .requiredOption(
      "--type <type>",
      "Test type (latency, throughput, errors)",
      "latency"
    )
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.log(
          chalk.white(`Duration: ${chalk.cyan(testResult.duration)} seconds`)
        );
        console.log("");
        console.log(chalk.yellow("Performance Metrics:"));
        console.log(
          chalk.white(
            `  Total Requests: ${chalk.cyan(testResult.results.totalRequests)}`
          )
        );
        console.log(
          chalk.white(
            `  Success Rate: ${chalk.cyan(
              (
                (testResult.results.successfulRequests /
                  testResult.results.totalRequests) *
                100
              ).toFixed(1)
            )}%`
          )
        ...
        console.log(
          chalk.white(
            `  Avg Latency: ${chalk.cyan(
              testResult.results.averageLatency.toFixed(2)
            )} ms`
          )
        );
        console.log(
          chalk.white(
            `  Min Latency: ${chalk.cyan(
              testResult.results.minLatency.toFixed(2)
            )} ms`
          )
        );
        console.log(
          chalk.white(
            `  Max Latency: ${chalk.cyan(
              testResult.results.maxLatency.toFixed(2)
            )} ms`
          )
        );
        console.log(
          chalk.white(
            `  P95 Latency: ${chalk.cyan(
              testResult.results.p95Latency.toFixed(2)
            )} ms`
          )
        );
        console.log(
          chalk.white(
            `  Requests/sec: ${chalk.cyan(
              testResult.results.requestsPerSecond.toFixed(2)
            )}`
          )
        );
        console.log("");
        ...
        console.log(
          chalk.white(
            `  View detailed results: ${chalk.cyan(
              `apigladiator results --test-id ${testResult.id}`
            )}`
          )
        );
        console.log(
          chalk.white(
            `  Compare with another API: ${chalk.cyan(
              `apigladiator compare --api-id ${api.id} --versus ANOTHER_API_ID`
            )}`
          )
        );
      } catch (error) {
